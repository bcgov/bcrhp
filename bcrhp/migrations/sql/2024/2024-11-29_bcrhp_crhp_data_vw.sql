...
select distinct i.resourceinstanceid,
                i.descriptors as site_descriptors,
                sn.site_names,
                bn.borden_number->'en'->>'value' as borden_number,
                sos.sos,
                addr.addresses,
                st_srid(sb.site_boundary) as site_boundary,
                st_astext(sb.site_boundary) as boundary_geojson,
                st_x(st_centroid(sb.site_boundary)) as site_centroid_longitude,
                st_y(st_centroid(sb.site_boundary)) as site_centroid_latitude,
                st_area(sb.site_boundary::geography) as area_sqm,
                hc.heritage_categories,
                hf.heritage_functions,
                se.significant_events,
                ca.construction_actors,
                ht.heritage_themes,
                __arches_get_concept_label(br.registration_status) as registration_status,
                (select jsonb_agg(child_label) from get_uuid_lookup_table('Registry Type') where child_value_uuid = any(br.registry_types)) as registry_types,
