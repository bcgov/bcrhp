...
import defaultCivicAddressWidgetTemplate from 'templates/views/components/widgets/civic-address-widget.htm';

export default ko.components.register('civic-address-widget', {
    viewModel: function (params) {
        ...
        this.textHasValue = function (textValue, languages = ['en']) {
            ...
            return languages.find(
                (language) =>
                    !!ko.unwrap(
                        ko.unwrap(textObject)
                            ? textObject[language].value
                            : null,
                    ),
            );
        };
