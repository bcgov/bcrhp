...
const BcrhpSiteViewModel = function (params) {
    var self = this;
    ...
    var getAllWidgets = function (card) {
        return _.flatten([
            ko.unwrap(card.tiles).length === 0 ? [] : ko.unwrap(card.widgets),
            _.map(card.cards(), (subcard) => {
                return getAllWidgets(subcard);
            }),
        ]);
    };
    ...
    this.helpactive = function (state) {
        this.helpenable(state);
    };
    ...
    var getTilesForAlias = function (node_alias) {
        ...
        let tiles = !widget
            ? []
            : _.filter(ko.unwrap(self.tiles), (tile) => {
    ...
    this.getCardForAlias = function (node_alias) {
        ...
        return widget
            ? _.find(self.report.cards, (card) => {
                  return card.nodegroupid === widget.node.nodegroup_id;
              })
            : null;
    };

    var getValueFromTile = function (tile, widget) {
        return !tile || !ko.unwrap(tile) || !widget
            ? null
            : ko.unwrap(ko.unwrap(tile).data[widget.node.nodeid]);
    };
    ...
    this.getValuesFromTiles = function (node_aliases) {
        ...
        _.each(ko.unwrap(tiles), (tile) => {
            ...
            if (_.some(_.values(tileValues))) {
                _.each(_.keys(tileValues), (k) => {
                    tileValues[k] = ko.observable(tileValues[k]);
                });
    ...
    this.textHasValue = function (textValue, languages = ['en']) {
        ...
        return !!languages.find(
            (language) =>
                !!ko.unwrap(
                    ko.unwrap(textObject) ? textObject[language].value : null,
                ),
        );
    };
    ...
    this.getResourceInstanceValues = function (aliases) {
        ...
        _.each(ko.unwrap(tiles), (tile) => {
            _.each(widgets, (widget) => {
                ...
                if (!!value) {
                    resourceValues.push({
                        [widget.alias]: ko.observable(value),
                    });
                }
            });
        });
    ...
    this.getLegislativeAct = function (relatedActObject) {
        ...
        if (
            self.report.graph.slug === 'heritage_site' &&
            self.tiles().length > 0
        ) {
            ...
            $.ajax({
                ...
                if (!!data && data.length > 0) {
                    authority.display_value(
                        `${data[0].authority}, ${data[0].recognition_type}`,
                    );
                    if (data[0].recognition_type_definition) {
                        authority.definition(
                            data[0].recognition_type_definition.value,
                        );
                    }
                }
            });
    ...
    this.submittedSites = ko.computed(function () {
        ...
        _.each(ko.unwrap(tiles), (tile) => {
            ...
            if (!!opVal || !!infoVal || !!siteVal)
                values.push({
                    requested_operation: ko.observable(opVal),
    ...
    this.getCardWithHierNames = function (parentCard, cardNames) {
        ...
        return self.getCardWithHierNames(
            namedCard,
            _.last(cardNames, cardNames.length - 1),
        );
    };
    ...
    this.initCarousel = function (parent) {
        ...
        $(parent).slick({
            ...
            // autoplaySpeed: 6000,
            prevArrow:
                "<button type='button' class='slick-prev pull-left'><i class='fa fa-chevron-left' aria-hidden='false'></i></button>",
            nextArrow:
                "<button type='button' class='slick-next pull-right'><i class='fa fa-chevron-right' aria-hidden='false'></i></button>",
        });
    };
};

export default BcrhpSiteViewModel;
